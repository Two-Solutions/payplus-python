...
class SQLAlchemyStorage(StorageBackend):
    ...
    async def create_tables(self) -> None:
        ...
        Table(
            ...
            Column("payplus_card_token", String(100)),
            Column("page_request_uid", String(100)),
            Column("payment_page_link", String(500)),
...
class MongoDBStorage(StorageBackend):
    ...
    @property
    def _subscriptions(self) -> Any:
        return self.db[f"{self.prefix}subscriptions"]

    async def create_indexes(self) -> None:
        """Create MongoDB indexes."""
        await self._customers.create_index("email", unique=True)
        await self._subscriptions.create_index("customer_id")
        await self._subscriptions.create_index("status")
        await self._subscriptions.create_index("current_period_end")
        await self._subscriptions.create_index("page_request_uid")
        await self._subscriptions.create_index("payplus_recurring_uid")

    async def save_customer(self, customer: Customer) -> None:
        """Save a customer."""
        data = customer.model_dump()
        data["_id"] = data.pop("id")
        await self._customers.replace_one({"_id": data["_id"]}, data, upsert=True)

    async def get_customer(self, customer_id: str) -> Optional[Customer]:
        """Get a customer by ID."""
        doc = await self._customers.find_one({"_id": customer_id})
        if doc:
            doc["id"] = doc.pop("_id")
            return Customer(**doc)
        return None
    ...
    async def get_customer_by_email(self, email: str) -> Optional[Customer]:
        """Get a customer by email."""
        doc = await self._customers.find_one({"email": email})
        if doc:
            doc["id"] = doc.pop("_id")
            return Customer(**doc)
        return None
    ...
    async def save_tier(self, tier: Tier) -> None:
        ...
        await self._tiers.replace_one({"_id": data["_id"]}, data, upsert=True)
    ...
    async def list_tiers(self, active_only: bool = True) -> list[Tier]:
        ...
        cursor = self._tiers.find(query).sort("display_order", 1)
        tiers = []
    ...
    async def save_subscription(self, subscription: Subscription) -> None:
        ...
        await self._subscriptions.replace_one({"_id": data["_id"]}, data, upsert=True)

    async def get_subscription(self, subscription_id: str) -> Optional[Subscription]:
        """Get a subscription by ID."""
        doc = await self._subscriptions.find_one({"_id": subscription_id})
        if doc:
            doc["id"] = doc.pop("_id")
    ...
    async def list_subscriptions_by_customer(
        ...
        cursor = self._subscriptions.find(query).sort("created_at", -1)
        subscriptions = []
        async for doc in cursor:
            doc["id"] = doc.pop("_id")
    ...
    async def get_subscription_by_page_request_uid(
        self, page_request_uid: str
    ) -> Optional[Subscription]:
        """Find a subscription by PayPlus page_request_uid."""
        doc = await self._subscriptions.find_one(
    ...
    async def get_subscription_by_recurring_uid(
        self, recurring_uid: str
    ) -> Optional[Subscription]:
        """Find a subscription by PayPlus recurring_uid."""
        doc = await self._subscriptions.find_one(
            {"payplus_recurring_uid": recurring_uid}
        )
        if doc:
            doc["id"] = doc.pop("_id")
            return Subscription(**doc)
